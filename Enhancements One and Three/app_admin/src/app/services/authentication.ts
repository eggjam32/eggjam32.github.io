...
export class AuthService {
  ...
  constructor(
    ...
    private tripDataService: ClientData
  ) { }
  ...
  // the key for our token 'snhu-token'
  public getToken(): string {
    let out: any;
    out = this.storage.getItem('snhu-token');
  ...
  public saveToken(token: string): void {
    this.storage.setItem('snhu-token', token);
  }
  // Logout of our application and remove the JWT from Storage
  public logout(): void {
    this.storage.removeItem('snhu-token');
  }
